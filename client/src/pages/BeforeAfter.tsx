...
const BeforeAfter = (): JSX.Element => {
  ...
  const [afterEnd, setAfterEnd] = useState<string>('');
  ...
  const handleQuickSetAfter = (months: number): void => {
  ...
  const handleQuickSetBefore = (months: number): void => {
  ...
  const calculateChange = (before: string | number, after: string | number): string => {
    const beforeVal = parseFloat(String(before)) || 0;
    const afterVal = parseFloat(String(after)) || 0;
  ...
  const getChangeArrow = (change: string): string => {
      ...
      return (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-3 rounded-lg shadow-lg">
          ...
          <p className={`text-sm font-semibold mt-1 ${getChangeColor(change)}`}>
            Change: {getChangeArrow(change)} {Math.abs(parseFloat(change))}%
          </p>
        </div>
      );
    }
              ...
              <div className="text-center py-2">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-800">
                  <span className={`text-2xl font-bold ${getChangeColor(calculateChange(data.before?.avg_authors, data.after?.avg_authors))}`}>
                    ...
                    {Math.abs(parseFloat(calculateChange(data.before?.avg_authors, data.after?.avg_authors)))}%
                  </span>
                </div>
