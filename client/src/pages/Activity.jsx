...
function Activity() {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      fetchDailyActivity(daysToShow).then(res => {
        ...
        if (selectedRepo !== 'all') {
          data = data.filter(d => d.repository_name === selectedRepo);
        }
  ...
  const aggregateByDate = () => {
    ...
    activityData.forEach(item => {
      ...
      dateMap[date].count += parseInt(item.total_commits || 0, 10);
      dateMap[date].lines_changed += parseInt(item.total_lines_changed || 0, 10);
  ...
  const heatmapData = aggregateByDate();
  ...
  const totalLinesChanged = heatmapData.reduce((sum, d) => sum + d.lines_changed, 0);
  ...
  const dayOfWeekData = () => {
    ...
    const dayMap = days.map(name => ({ name, commits: 0 }));
  ...
  const dayDistribution = dayOfWeekData();
