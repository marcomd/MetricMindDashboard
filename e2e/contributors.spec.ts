...
test.describe('Contributors Page', () => {
  ...
  test('should display contributors page with leaderboard', async ({ page }: { page: Page }) => {
    // Check page title
  ...
  test('should display top contributors podium', async ({ page }: { page: Page }) => {
  ...
  test('should search for contributors', async ({ page }: { page: Page }) => {
    ...
    if (await searchInput.count() > 0) {
      ...
      const initialCount: number = await tableRows.count();

      if (initialCount > 0) {
        // Type a search term (get first contributor name)
        const firstContributor: string | null = await tableRows.first().locator('td').first().textContent();
        const searchTerm: string = firstContributor?.substring(0, 3) || 'test';
  ...
  test('should change contributor limit', async ({ page }: { page: Page }) => {
    ...
    if (await limitButtons.count() > 0) {
      ...
      const countBefore: number = await tableRowsBefore.count();
  ...
  test('should display contributor statistics table', async ({ page }: { page: Page }) => {
    // Wait for table
    ...
    const headerCount: number = await headers.count();
    ...
    const cellText: string | null = await firstRow.locator('td').first().textContent();
  ...
  test('should filter by repository', async ({ page }: { page: Page }) => {
    ...
    if (await repoSelect.count() > 0) {
      ...
      // Get options
      const options: number = await repoSelect.locator('option').count();
  ...
  test('should display contributor charts', async ({ page }: { page: Page }) => {
  ...
  test('should sort table columns', async ({ page }: { page: Page }) => {
    // Wait for table
    await page.waitForSelector('table', { timeout: 10000 });
    ...
    if (await sortableHeaders.count() > 0) {
      ...
      const firstRowBefore: string | null = await page.locator('table tbody tr').first().textContent();
  ...
  test('should handle date range filtering', async ({ page }: { page: Page }) => {
