...
test.describe('Overview Page', () => {
  test('should load and display repository overview', async ({ page }: { page: Page }) => {
    await page.goto('/');
    ...
    const count: number = await statCards.count();
  ...
  test('should display repository cards', async ({ page }: { page: Page }) => {
    await page.goto('/');
    ...
    const count: number = await repoCards.count();
    expect(count).toBeGreaterThan(0);
  });

  test('should navigate to different pages', async ({ page }: { page: Page }) => {
    await page.goto('/');
  ...
  test('should toggle dark mode', async ({ page }: { page: Page }) => {
    await page.goto('/');
    ...
    const initialTheme: string | null = await htmlElement.getAttribute('class');
    ...
    const newTheme: string | null = await htmlElement.getAttribute('class');
